...
export async function recoverRuntimeSessions(): Promise<void> {
  await ensureTmuxAvailable();
  const rows: Array<{ db: DatabaseType; session: SessionRow }> = [];
  for (const project of allAppProjects()) {
    try {
      ...
      for (const session of sessions) {
        rows.push({ db, session });
      }
    } catch {
      // Ignore unavailable/corrupt project DBs during heartbeat recovery.
    }
  }
  ...
  heartbeatTimer = setInterval(async () => {
    ...
    for (const project of allAppProjects()) {
      try {
        ...
        // Ignore unavailable/corrupt project DBs for this pass.
      }
    }
